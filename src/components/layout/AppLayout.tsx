      ...
      <nav className="flex-1 p-3 space-y-0.5">
        {navItems.map((item) => (
          <NavLink
            ...
            onClick={onNavigate}
            className={({ isActive }) => {
              const spark = "spark" in item && item.spark;
              if (spark && isActive) return "flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-violet-600 to-indigo-600 text-white transition-all duration-150";
              if (spark) return "flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-violet-700 hover:bg-violet-50 transition-all duration-150";
