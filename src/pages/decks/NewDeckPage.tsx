...
type CreationMode = "ai" | "custom_html";
...
export default function NewDeckPage() {
  ...
  async function handleSubmitCustomHtml() {
    if (!title.trim() || !customHtml.trim()) return;
    const deck = await createDeck.mutateAsync({
      title: title.trim(),
      topic: title.trim(),
      custom_html: customHtml.trim(),
    });
    navigate(`/decks/${deck.id}`);
  }
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
          {([
            ...
            { value: "custom_html", label: "</> Paste HTML" },
          ] as const).map(m => (
            <button
              key={m.value}
              type="button"
              ...
              className={[
                ...
                creationMode === m.value ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700",
              ].join(" ")}
            >
              {m.label}
            </button>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-4 space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400">{customHtml.length.toLocaleString()} chars</span>
                </div>
                <textarea
                  value={customHtml}
                  ...
                  className="w-full text-xs font-mono px-3 py-3 border border-slate-200 rounded-xl outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-y bg-slate-50 text-slate-800"
                />
                <p className="text-xs text-slate-400">
                  Paste any HTML — a Gamma export, Canva embed, hand-coded slide, anything. It will be served as-is.
                </p>
              </div>
            </div>

            <button
              onClick={handleSubmitCustomHtml}
              disabled={createDeck.isPending || !title.trim() || !customHtml.trim()}
        ...
        {/* Step indicator — only for AI mode */}
        {creationMode === "ai" && (
